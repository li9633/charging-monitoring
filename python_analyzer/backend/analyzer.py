...
def analyze_offline_patterns():
    ...
    for pile_no in sorted(pile_hour_data.keys()):
        ...
        offline_rate = (total_offline / total_checks * 100) if total_checks > 0 else 0

        print(f"\n{'─' * 60}")
        print(f"桩号: {pile_no} | {location_display}")
        print(f"总检查: {total_checks} | 在线: {total_checks - total_offline} | 离线: {total_offline} | 离线率: {offline_rate:.1f}%")
...
def get_report_data(tag_filter=None, pile_no_filter=None):
    ...
    for pile_no in sorted(pile_hour_data.keys()):
        ...
        suspicious_ranges = _merge_suspicious_ranges(suspicious) if suspicious else ""
